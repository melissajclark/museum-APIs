...
artApp.displayPieces = function(pieces) {
	...
	$.ajax({
		...
		success: function(result) {  // when the ajax request comes back - run this code! (another word for success = callback)
			...
			var img = "<a href='" + artPiece.webImage.url + "'data-featherlight='image'><img class='artImage lazy artImageLink' data-original='" + artPiece.webImage.url + "'" + "src='" + artPiece.webImage.url + "'></a>";
